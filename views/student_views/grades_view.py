...
def GradesView(page: ft.Page, params: Params, basket: Basket) -> ft.View:
    async def add_grades():
        ...
        try:
            for grade in sub_db.database.get_student_grades(user_id=page.session.get('user_id')):
                await add_grade(f'Предмет: {grade[1]}', f"{grade[2]}", f"Дата оценки: {grade[3].strftime('%d-%m-%Y')}")
        except DontHaveGrades as err:
            no_grades.value = 'У вас нет оценок'
            no_grades.visible = True
            await page.update_async()

    async def add_grade(subject_title: str, grade_value: str, grade_date: str) -> None:
        """Add grade card"""
    ...
    async def tabs_changed(e: ft.ControlEvent) -> None:
        ...
        if status == 'Все':
            ...
            try:
                for _grade in sub_db.database.get_student_grade_for_exact_subject(e.page.session.get('username'),
                                                                                  status):
