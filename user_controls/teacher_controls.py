...
class TeacherSubjectsCard(ft.UserControl):
    def __init__(self, user_id, button_click):
        ...
        try:
            self.all_subjects_count = len(self.db.get_teacher_subjects(self.user_id))
        except DontHaveGrades as err:
    ...
    def create_subjects_controls(self) -> list[ft.Control]:
        ...
        count = 1
        try:
            for i in self.db.get_teacher_subjects(user_id=self.user_id):
                all_subjects.append(
                    ft.ListTile(
...
class TeacherExperience(ft.UserControl):
    ...
    def build(self):
        ...
        try:
            ...
            if information.teacher_description or information.teacher_experience is None:
